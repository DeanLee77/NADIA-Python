...
from node.Record import Record
...
class TopologicalSort:
    ...
    def bfs_topological_sort(node_dictionary: dict, node_id_dictionary: dict, dependency_matrix:[[]]) -> list:
        ...
        temp_list = list()
        s_list = TopologicalSort.filling_s_list(node_dictionary, node_id_dictionary, temp_list, copy_of_dependency_matrix)
        ...
        while len(s_list) > 0:
            ...
            for index in range(size_of_matrix):
                if (node_id is not index) and copy_of_dependency_matrix[node_id][index] is not 0:
                    copy_of_dependency_matrix[node_id][index] = 0 # this is to remove dependency from 'node' to child node with nodeId == 'i'
                    number_of_incoming_edge = size_of_matrix - 1 # from this line, it is process to check whether or not the child node with nodeId == 'i' has any other incoming dependencies from other nodes, and the reason for subtracting 1 from matrixSize is to exclude node itself count from the matrix size.
                    for second_index in range(size_of_matrix):
                        if (node_id is not second_index) and (copy_of_dependency_matrix[second_index][index] is not 0):
                            number_of_incoming_edge -= number_of_incoming_edge
                    if number_of_incoming_edge == 0: # there is no incoming dependencies for the node with nodeId == 'i'
                        TopologicalSort.s_list.append(node_dictionary[node_id_dictionary[index]])

        check_dag = False
        for i in range(size_of_matrix):
            for j in range(size_of_matrix):
                if (i != j) and copy_of_dependency_matrix[i][j] != 0:
                    check_dag = True
                    break
    ...
    @staticmethod
    def filling_s_list(node_dictionary:dict, node_id_dictionary: dict, temp_list: list, dependency_matrix: [[]]) -> list:
        size_of_matrix = len(dependency_matrix)
        for child_row in range(size_of_matrix):
            count = 0
            for parent_col in range(size_of_matrix):
                if (dependency_matrix[parent_col][child_row] == 0) and (parent_col != child_row):
                    count += count
                else:
                    continue
            if count == size_of_matrix -1: # exclude its own dependency
                temp_node_name = node_id_dictionary[child_row]
    ...
    @staticmethod
    def create_copy_of_dependency_matrix( dependency_matrix:[[]], size_of_matrix) -> [[]]:
        copy_of_dependency_matrix = [[0 for x in range(size_of_matrix)] for y in range(size_of_matrix)]
        for parent_col in range(size_of_matrix):
            for child_row in range(size_of_matrix):
                copy_of_dependency_matrix[parent_col][child_row] = dependency_matrix[parent_col][child_row]
    ...
    def dfs_topological_sort(node_dictionary: dict, node_id_dictionary:dict, dependency_matrix:[[]]) -> list:
        ...
        while len(s_list) > 0:
            ...
            for index in range(len(copy_of_dependency_matrix)):
                if copy_of_dependency_matrix[node_id][index] != 0:
                    child_id_list.append(index)

            for id in range(len(child_id_list)):
                current_node = node_dictionary[node_id_dictionary[id]]
                if id not in visited_list:
                    sorted_list.append(current_node)
                    visited_list.append(id)
                    TopologicalSort.deepening(node_dictionary , node_id_dictionary, copy_of_dependency_matrix, sorted_list, visited_list, id)

        return sorted_list

    @staticmethod
    def deepening(node_dictionary: dict, node_id_dictionary: dict, dependency_matrix:[[]], sorted_list: list, visited_list: list, child_id):
        child_id_list = list()
        for i in range(len(dependency_matrix)):
            if dependency_matrix[child_id][i] != 0:
                child_id_list.append(i)

        for index in range(len(child_id_list)):
            current_node = node_dictionary[node_id_dictionary[index]]

            if index not in visited_list:
                sorted_list.append(current_node)
                visited_list.append(index)

            TopologicalSort.deepening(node_dictionary, node_id_dictionary, dependency_matrix, sorted_list, visited_list, id)
    ...
    #
    @staticmethod
    def dfs_topological_sort(node_dictionary: dict, node_id_dictionary: dict, dependency_matrix: [[]], record_dictionary_of_node: dict) -> list:
        sorted_list = list()
    ...
    def visit(node: Node, sorted_list: list, record_dictionary_of_nodes: dict, node_dictionary: dict, node_id_dictionary: dict, visited_node_list: list, dependency_matrix:[[]]) -> list:
        if node is not None:
            ...
            or_out_dependency = list(filter(lambda index: (dependency_matrix_as_list[index] & or_dependency_type) == or_dependency_type,
                                            list([0 for x in range(size_of_dependency_matrix_as_list)])))
            and_out_dependency = list(filter(lambda index: (dependency_matrix_as_list[index] & and_dependency_type) == and_dependency_type,
                                             list([0 for x in range(size_of_dependency_matrix_as_list)])))

            if (len(or_out_dependency) != 0) or (len(and_out_dependency) != 0):
                child_rule_list = list()
                for item in list(filter(lambda child_index: dependency_matrix_as_list[child_index] != 0, list([0 for x in range(size_of_dependency_matrix_as_list)]))):
                ...
                if (len(or_out_dependency) != 0) and (len(and_out_dependency) == 0):
                    while len(child_rule_list) != 0:
                        ...
                        the_most_positive = TopologicalSort.find_the_most_positive(child_rule_list, record_dictionary_of_nodes, dependency_matrix_as_list)
                        ...
                        if the_most_positive not in visited_node_list:
                            ...
                            sorted_list = TopologicalSort.visit(the_most_positive, sorted_list,
                                                                ...
                                                                node_id_dictionary, visited_node_list, dependency_matrix)
                            ...
                            if (len(or_out_dependency) == 0) and (len(and_out_dependency) != 0):
                                ...
                                while len(child_rule_list) != 0:
                                    the_most_negative = TopologicalSort.find_the_most_negative(child_rule_list, record_dictionary_of_nodes, dependency_matrix_as_list)
    ...
    @staticmethod
    def find_the_negative(child_node_list: list, record_dictionary_of_nodes: dict, dependency_matrix_as_list: list = None) -> Node:
    ...
    def analysis(the_result: float, the_most_possibility: float, yes_count_no_count: int, summation: int) -> bool:
        ...
        elif (the_result >= the_most_possibility) and (the_result == 0 and the_most_possibility == 0) and (yes_count_no_count > summation):
            highly_possible = True
        elif (the_result >= the_most_possibility) and (the_result == 0) and (yes_count_no_count == -1) and (summation <= 0) and (summation != -1):
            highly_possible = True
        elif (the_result >= the_most_possibility) and (the_result == 0) and (yes_count_no_count == -1) and (summation <= 0) and (summation != -1):
            highly_possible = True
